...
module.exports = function () {
    ...
    this.When(/^enter my lastname$/, async function () {
        let afterName = await $('#details > div:nth-child(3) > input:nth-child(2)')
        await afterName.sendKeys('Hej');
        await sleep(1500);
    });

    this.When(/^enter telephonenumber$/, async function () {
        let afterName = await $('#details > div:nth-child(3) > input:nth-child(3)')
        await afterName.sendKeys('0721239876');
        await sleep(1500);
    });

    this.When(/^enter my email$/, async function () {
        let afterName = await $('#details > div:nth-child(3) > input:nth-child(4)')
        await afterName.sendKeys('test@test.se');
        await sleep(1500);
    });

    this.When(/^enter street address$/, async function () {
        let afterName = await $('#details > div:nth-child(3) > input:nth-child(5)')
        await afterName.sendKeys('12312333');
        await sleep(1500);
    });

    this.When(/^enter zip code$/, async function () {
        let zipCode = await $('#details > div:nth-child(3) > input:nth-child(6)')
        await zipCode.sendKeys('22123');
        await sleep(1500);
    });

    this.When(/^enter city name$/, async function () {
        let zipCode = await $('#details > div:nth-child(3) > input:nth-child(7)')
        await zipCode.sendKeys('Malmö');
        await sleep(1500);
    });

    this.Then(/^click Skicka button to place an order\.$/, async function () {
